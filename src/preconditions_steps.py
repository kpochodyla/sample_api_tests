...
def register_user(base_url: str, user: dict, data: dict) -> None:
    # Step 1: Send a POST request to register a users
    response = step.post_request(base_url + "/register", data)
    ...
    step.check_values_are_equal(user["id"], response_data["id"])

@allure.step("Register User")
def login_user(base_url: str, user: dict, data: dict) -> None:
    # Step 1: Send a POST request to login a users
    response = step.post_request(base_url + "/login", data)
    # Step 2: Verify that the response code is 200 OK
    step.check_status(response, 200)
    # Step 3: Verify that the response content returned is in JSON format
    step.check_headers(response, "Content-type", "application/json; charset=utf-8")
